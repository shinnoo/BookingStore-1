...
export class FullnameComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.fullnameService
      ...
      .subscribe(
        (res: HttpResponse<IFullname[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInFullnames(): void {
    this.eventSubscriber = this.eventManager.subscribe('fullnameListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IFullname[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
