...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      createAt: payment.createAt && payment.createAt.isValid() ? payment.createAt.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.createAt = payment.createAt ? moment(payment.createAt) : undefined;
      });
    }
    return res;
  }
